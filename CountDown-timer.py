...
def Main():
    ...
    Stop = Button(Bottom, text='Stop',
                  command=stopWatch.Stop, width=10, height=2)
    Stop.pack(side=LEFT)
    Reset = Button(Bottom, text='Reset',
                   command=stopWatch.Reset, width=10, height=2)
    Reset.pack(side=LEFT)
    Exit = Button(Bottom, text='Close',
                  command=stopWatch.Exit, width=10, height=2)
    Exit.pack(side=LEFT)
    Title = Label(Top, text="Stopwatch For Beginners",
...
class StopWatch(Frame):
    ...
    def MakeWidget(self):
        timeText = Label(self, textvariable=self.timestr, font=(
            "times new roman", 50), fg="green", bg="black")
